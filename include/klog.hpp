...
namespace klog
{
    ...
    template <class PreFmt= KLogPrefix<KLOG_PREFIX_TIME> > 
        class KLog
        {
            ...
            struct DebugLogger {
                ...
                template <class T>
                    FlowHelper operator<<(const T& log) {
                        ...
                        return logger.debug_logger() << PreFmt()(KLOG_LEVEL_DEBUG) << log;
#else 
                        return logger.null_logger() << log;
#endif 
                    }
            ...
            struct InfoLogger {
                ...
                template <class T>
                    FlowHelper operator<<(const T& log) {
                        ...
                        return logger.info_logger() << PreFmt()(KLOG_LEVEL_INFO) << log;             
#else 
                        return logger.null_logger() << log;
#endif  
                    }
            ...
            struct WarnLogger {
                ...
                template <class T>
                    FlowHelper operator<<(const T& log) {
                        ...
                        return logger.warn_logger() << PreFmt()(KLOG_LEVEL_WARN) << log;         
#else 
                        return logger.null_logger() << log;
#endif  
                    }
            ...
            struct ErrorLogger {
                ...
                template <class T>
                    FlowHelper operator<<(const T& log) {
#if KLOG_LEVEL >= KLOG_LEVEL_WARN   
                        return logger.error_logger() << PreFmt()(KLOG_LEVEL_ERROR) << log;                         
#else 
                        return logger.null_logger() << log;
#endif 
                    }
...
inline   klog::KLog<>& dput(Args... args) {
    return klog::KLog<>::instance().dput(args ...);
}
#else 
template <class... Args>
inline klog::KLog<>& dput(Args... args) {
...
inline klog::KLog<>& iput(Args... args) {
    return klog::KLog<>::instance().iput(args ...);
}
#else 
template <class... Args>
inline klog::KLog<>& iput(Args... args) {
...
inline klog::KLog<>& wput(Args... args) {
    return klog::KLog<>::instance().wput(args ...);
}
#else 
template <class... Args>
inline klog::KLog<>& wput(Args... args) {
...
inline klog::KLog<>& eput(Args... args) {
    return klog::KLog<>::instance().eput(args ...);
}
#else 
template <class... Args>
inline klog::KLog<>& eput(Args... args) {
